...
export const GAME_CATALOG: GameCatalogItem[] = [
  ...
  },
  {
    ...
    variants: [
      { id: 'classic', name: 'Classic', icon: 'CK', desc: 'Standard capture-and-crown checkers.', path: '/games/checkers', previewSteps: ['Advance', 'Jump', 'Crown'] },
      { id: 'anti', name: 'Anti-Checkers', icon: 'AC', desc: 'Losing all pieces or running out of moves wins.', path: '/games/checkers/anti', rulesTitle: 'Anti-Checkers Rules', rules: ['Normal movement and forced captures still apply.', 'If you lose all pieces or cannot move, you win instead of lose.', 'Use sacrifices to empty your side before your opponent can do the same.'], tips: ['Offer captures that leave your opponent with awkward material.', 'Avoid crowning unless it helps you shed pieces faster.'], previewSteps: ['Sacrifice', 'Force takes', 'Empty first'] },
      { id: 'zombie', name: 'Zombie Checkers', icon: 'ZC', desc: 'Captured pieces convert to your side instead of leaving the board.', path: '/games/checkers/zombie', rulesTitle: 'Zombie Rules', rules: ['Jumped pieces become yours instead of being removed.', 'Converted pieces can be used on later turns.', 'Win by taking over the board or trapping the opponent.'], tips: ['Chain conversions near the center for maximum reach.', 'A bad capture can hand the opponent a stronger army.'], previewSteps: ['Jump', 'Convert', 'Swarm'] },
      { id: 'minefield', name: 'Minefield', icon: 'MC', desc: 'Both players secretly place traps before the first move.', path: '/games/checkers/minefield', rulesTitle: 'Minefield Rules', rules: ['Before play, each player secretly marks up to three mine squares.', 'When an opponent lands on your mine, the landing area explodes.', 'Explosions remove nearby pieces and clear the mine.'], tips: ['Put mines where forced jumps are likely to end.', 'Leave yourself escape squares around suspected traps.'], previewSteps: ['Hide mines', 'Lure jumps', 'Trigger blast'] },
      { id: 'vip', name: 'VIP', icon: 'VC', desc: 'Choose one secret protected piece; capturing it wins instantly.', path: '/games/checkers/vip', rulesTitle: 'VIP Rules', rules: ['Before play, each player secretly chooses one of their pieces as VIP.', 'Capturing the opposing VIP wins immediately.', 'VIP identity is hidden from the opponent until it matters.'], tips: ['Move decoys like they matter.', 'Do not strand your VIP without a trade route.'], previewSteps: ['Choose VIP', 'Hide intent', 'Capture target'] },
      { id: 'portal', name: 'Portal', icon: 'PC', desc: 'Landing on paired portal squares teleports the moving piece.', path: '/games/checkers/portal', rulesTitle: 'Portal Rules', rules: ['Portal squares appear in pairs near the center.', 'A piece that lands on a portal moves to its paired portal.', 'Portal travel ends the jump sequence for that turn.'], tips: ['Use portals to dodge blocks or create sudden crowning routes.', 'Watch portal exits before forcing captures.'], previewSteps: ['Find portal', 'Teleport', 'Reposition'] },
    ],
    playerCount: 2,
    playerLabel: '2 Players',
    difficulty: 'medium',
  ...
  },
  {
    ...
    variants: [
      { id: 'classic', name: 'Classic', icon: 'D4', desc: 'Standard gravity-based four-in-a-row.', path: '/games/drop-four', previewSteps: ['Drop', 'Stack', 'Connect'] },
      { id: 'wrecking-ball', name: 'Wrecking Ball', icon: 'WB', desc: 'Each player gets one detonating token that removes adjacent pieces.', path: '/games/drop-four/wrecking-ball', rulesTitle: 'Wrecking Ball Rules', rules: ['Each player has one Wrecking Ball token.', 'It drops like normal, then removes itself and all adjacent tokens.', 'Gravity compacts the affected columns before wins are checked.'], tips: ['Use the blast to break a loaded center stack.', 'A detonation can accidentally create a line after gravity settles.'], previewSteps: ['Arm token', 'Detonate', 'Settle'] },
      { id: 'popout', name: 'PopOut', icon: 'PO', desc: 'Remove one of your own bottom tokens instead of dropping.', path: '/games/drop-four/popout', rulesTitle: 'PopOut Rules', rules: ['On your turn, drop normally or pop one of your own bottom tokens.', 'The remaining pieces in that column slide down.', 'Wins are checked after the popout settles.'], tips: ['Popouts can turn old stacks into sudden diagonals.', 'Do not pop a support piece that gifts the opponent a line.'], previewSteps: ['Drop', 'Pop base', 'Shift line'] },
      { id: 'gravity-flip', name: 'Gravity Flip', icon: 'GF', desc: 'Each player can invert the grid once.', path: '/games/drop-four/gravity-flip', rulesTitle: 'Gravity Flip Rules', rules: ['Each player has one Flip Gravity power.', 'Flipping turns the board upside down and reverses gravity.', 'The board compacts under the new gravity before wins are checked.'], tips: ['Flip when the opponent stack depends on old support.', 'Plan where your tokens land after inversion.'], previewSteps: ['Build', 'Flip', 'Reconnect'] },
      { id: 'battleship-drop', name: 'Battleship Drop', icon: 'BD', desc: 'The lower half of the grid is masked until pieces rise above it.', path: '/games/drop-four/battleship-drop', rulesTitle: 'Battleship Drop Rules', rules: ['The lower half of the grid is hidden from both players.', 'Drops still resolve server-side with normal gravity.', 'Visible top cells reveal normally when stacks rise above the screen.'], tips: ['Track column counts carefully.', 'Use opponent hesitation as a memory clue.'], previewSteps: ['Drop blind', 'Remember stacks', 'Reveal top'] },
      { id: 'heavy-token', name: 'Heavy Token', icon: 'HT', desc: 'Each player gets one token that crushes the piece beneath it.', path: '/games/drop-four/heavy-token', rulesTitle: 'Heavy Token Rules', rules: ['Each player has one Heavy token.', 'When dropped, it crushes exactly one non-heavy token beneath it.', 'Heavy tokens cannot crush other Heavy tokens.'], tips: ['Save the heavy token for a defended column.', 'Crushing can open space and create a new landing pattern.'], previewSteps: ['Arm heavy', 'Crush', 'Connect'] },
    ],
    playerCount: 2,
    playerLabel: '2 Players',
    difficulty: 'medium',
